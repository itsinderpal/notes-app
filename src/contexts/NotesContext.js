...
const noteReducer = (state, action) => {
	switch (action.type) {
		...
		case "editNote":
			...
			const editedNote = { ...state.note, [name]: value, modifiedDate: new Date().toDateString() };
		...
		case "searchNote":
			...
			if (searchQuery) {
				const queryNotes = state.notes.filter(
					(n) => n.title.toLowerCase().indexOf(searchQuery) !== -1
				);
